...
Use MCP `remove_keywords` (or `./scripts/astro-optimize.py --prune` in repos with that script). Remove keywords that:
...
- Test phrases or over-niche long tails

```bash
# Bulk remove (max 100 per call)
curl -s -X POST http://127.0.0.1:8089/mcp -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"remove_keywords","arguments":{"appId":"APP_ID","store":"us","keywords":["bad keyword","дневник"]}}}'
```

Or from repo root: `./scripts/astro-optimize.py --prune`
...
[ ] Prune junk keywords (remove_keywords or astro-optimize.py --prune)
